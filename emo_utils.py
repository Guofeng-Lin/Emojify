...
def sentences_to_indices(X, word_to_index, max_len):
    ...
    :param max_len: maximum number of words in a sentences. We assume every sentences in X is no longer than this.
    ...
    for sentence_offset in range(m):

        # Convert the ith training sentence in lower case and split it into words.
        sentence_words = X[sentence_offset].lower().split()

        # Initialize word_offset = 0
        word_offset = 0

        # Loop over the words of sentence_words
        for word in sentence_words:
            if word in list(word_to_index.keys()):
                X_indices[sentence_offset, word_offset] = word_to_index[word]
                word_offset += 1
